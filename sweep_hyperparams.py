...
Trains multiple configurations and evaluates each on a test set.
Results are saved to a JSON file for comparison.

Usage:
    python sweep_hyperparams.py --model path/to/model --data-dir path/to/training --test-file path/to/test.jsonl
    python sweep_hyperparams.py --model path/to/model --data-dir training --quick
"""
...
def run_evaluation(model_path: str, adapter_path: str, test_file: str, max_examples: int = 100, use_4bit: bool = False) -> dict:
    ...
    for line in result.stdout.split('\n'):
        if "Response Type Accuracy:" in line or "Type Accuracy:" in line:
            try:
                metrics["type_accuracy"] = float(line.split(':')[1].strip().rstrip('%'))
            except:
                pass
        if "Exact match:" in line or "Command Accuracy:" in line:
            try:
...
def main():
    ...
    parser.add_argument("--test-file", "-t", required=True, help="Test data JSONL file")
    ...
    if args.output_dir:
        ...
        output_dir = Path.cwd() / "sweep_results"
